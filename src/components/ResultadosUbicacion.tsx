      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-3">
        ...
        <form action="/buscar" method="GET" className="flex gap-2 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-60">
            <input name="q" defaultValue={q} placeholder="Buscar..." className="w-full border rounded-lg pl-9 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-yellow" />
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
          </div>
          <button type="submit" className="bg-brand-yellow text-brand-blue px-5 rounded-lg font-bold text-sm hover:bg-yellow-400 transition whitespace-nowrap">Buscar</button>
        </form>
      </div>
