...
export default function PublicarPage() {
  ...
  const camposEspeciales = sub?.campos.map(c => ({
    ...c,
    options: c.label === 'Año' ? years : (c.options || []),
  })) || []
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < Math.min(files.length, 10 - imagenes.length); i++) {
      ...
      reader.onloadend = () => { newImages.push(reader.result as string) }
      reader.readAsDataURL(files[i])
    }
    setTimeout(() => setImagenes(prev => [...prev, ...newImages]), 100)
  }
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await supabase.from('productos').insert({
      titulo, descripcion, categoria, subcategoria, marca: marca || null,
      estado: estadoProd, precio_usd: parseFloat(precioUsd) || null,
      ubicacion_estado: ubicacionEstado, ubicacion_ciudad: ubicacionCiudad,
      especificaciones: JSON.stringify(specs), activo: true, destacado: false,
    })
    if (error) alert('Error: ' + error.message)
    else { alert('¡Publicado con éxito!'); window.location.href = '/dashboard' }
    setLoading(false)
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <p className="text-gray-500 mb-8">Completa la información. Es gratis.</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
        ...
        {/* PASO 1: Categoría → Subcategoría → Marca */}
        {step === 1 && (
          <div className="space-y-5 animate-fadeIn">
              ...
              <>
                ...
                {subcategoria && sub?.marcas.length && sub.marcas.length > 0 && (
                  <div>
                    ...
                    <input type="text" value={marca} onChange={e => setMarca(e.target.value)} placeholder="Escribe la marca..." list={`pub-${categoria}-marcas`} className="w-full border border-gray-300 rounded-lg px-4 py-3 text-gray-800" />
            ...
            <button onClick={() => setStep(2)} disabled={!categoria || !subcategoria} className="w-full bg-brand-blue text-white py-3 rounded-lg font-bold hover:bg-blue-900 transition disabled:opacity-50">Siguiente</button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-5 animate-fadeIn">
            ...
            <div>
              ...
              <input type="text" value={titulo} onChange={e => setTitulo(e.target.value)} placeholder="Describe tu producto claramente..." maxLength={100} required className="w-full border border-gray-300 rounded-lg px-4 py-3 text-gray-800" />
                ...
                {camposEspeciales.map(campo => (
                  <div key={campo.label}>
                    ...
                    {campo.type === 'select' ? (
                      ...
                      <input type={campo.type} value={specs[campo.label] || ''} onChange={e => handleSpecChange(campo.label, e.target.value)} placeholder={campo.placeholder} className="w-full border border-gray-300 rounded-lg px-4 py-3 text-gray-800" />
                    )}
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <textarea value={descripcion} onChange={e => setDescripcion(e.target.value)} rows={5} maxLength={2000} required className="w-full border border-gray-300 rounded-lg px-4 py-3 text-gray-800 resize-none" placeholder="Describe el estado, características, accesorios incluidos..." />
            ...
            <div>
              ...
              <input type="number" value={precioUsd} onChange={e => setPrecioUsd(e.target.value)} placeholder="Ej: 250" min="0" step="0.01" required className="w-full border border-gray-300 rounded-lg px-4 py-3 text-gray-800" />
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input type="text" value={ubicacionCiudad} onChange={e => setUbicacionCiudad(e.target.value)} placeholder="Ciudad" required className="w-full border border-gray-300 rounded-lg px-3 py-3 text-gray-800" />
              </div>
            </div>

            <div className="flex gap-3">
              <button onClick={() => setStep(1)} className="px-6 py-3 rounded-lg font-medium border border-gray-200 hover:bg-gray-50">← Atrás</button>
              <button onClick={() => setStep(3)} disabled={!titulo || !descripcion || !estadoProd || !precioUsd} className="flex-1 bg-brand-blue text-white py-3 rounded-lg font-bold hover:bg-blue-900 transition disabled:opacity-50">Siguiente</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-5 animate-fadeIn">
            <h2 className="text-xl font-bold text-gray-900">📷 Añade fotos</h2>
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep(2)} className="px-6 py-3 rounded-lg font-medium border border-gray-200 hover:bg-gray-50">← Atrás</button>
        ...
        {step === 4 && (
          <div className="space-y-5 animate-fadeIn">
            ...
            <div className="border rounded-lg p-5 space-y-3">
              {imagenes.length > 0 && <div className="aspect-video bg-gray-100 rounded-lg overflow-hidden"><img src={imagenes[0]} alt="" className="w-full h-full object-cover" /></div>}
              ...
              <div className="mt-4 p-4 bg-gray-50 rounded-lg"><p className="text-sm text-gray-600"><strong>Descripción:</strong></p><p className="text-sm text-gray-700 mt-1">{descripcion}</p></div>
            </div>
            <div className="flex gap-3">
